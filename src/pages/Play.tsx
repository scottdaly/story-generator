...
const API_URL = 'http://localhost:3000/generate-story';
...
function Play() {
  ...
  const generateStory = async () => {
    ...
    const requestBody = {
      prompt: story === '' ? initialPrompt : prompt,
      genre,
      length,
      character
    };
    ...
    try {
      ...
      setError(err.message || 'Failed to connect to the story generator. Is the backend running?');
    } finally {
      setIsLoading(false);
    }
  }
...
<div role="status">
    <svg aria-hidden="true" className="w-8 h-8 text-gray-300 animate-spin dark:text-gray-300 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen text-gray-800 p-4">
      ...
      <div className="w-full max-w-2xl bg-white p-6 rounded-lg shadow-md mb-4">
        <p className="text-gray-600 whitespace-pre-wrap">{isLoading ? "Generating..." : story}</p>
      </div>
